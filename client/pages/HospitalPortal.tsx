...
import { 
  Heart, ArrowLeft, Plus, AlertTriangle, TrendingDown, TrendingUp, 
  Package, Calendar, Clock, Users, Hospital, Activity, 
  RefreshCw, Download, Send, CheckCircle, XCircle
} from 'lucide-react';
...
const bloodInventory = [
  { type: 'O+', units: 45, low: 20, critical: 10, expiring: 3, status: 'adequate' },
  { type: 'O-', units: 8, low: 15, critical: 8, expiring: 1, status: 'critical' },
  { type: 'A+', units: 22, low: 18, critical: 10, expiring: 2, status: 'adequate' },
  { type: 'A-', units: 12, low: 12, critical: 6, expiring: 0, status: 'low' },
  { type: 'B+', units: 18, low: 15, critical: 8, expiring: 1, status: 'adequate' },
  { type: 'B-', units: 5, low: 10, critical: 5, expiring: 0, status: 'critical' },
  { type: 'AB+', units: 14, low: 8, critical: 4, expiring: 1, status: 'adequate' },
  { type: 'AB-', units: 7, low: 6, critical: 3, expiring: 0, status: 'adequate' }
];
          ...
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div className="flex items-center space-x-4">
                ...
                <div>
                  <h1 className="text-2xl font-bold text-hope-red">{hospitalInfo.name}</h1>
                  <p className="text-muted-foreground">{hospitalInfo.location}</p>
                  <div className="flex items-center space-x-2 mt-1">
                    <Badge variant="outline" className="border-success text-success">
                      {hospitalInfo.verified ? 'Verified Partner' : 'Pending Verification'}
                    </Badge>
                    <span className="text-sm text-muted-foreground">ID: {hospitalInfo.id}</span>
                  </div>
                </div>
              </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
          ...
          <Card className="border-0 shadow-lg">
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-muted-foreground text-sm">Critical Types</p>
                  <p className="text-3xl font-bold text-destructive">
                    {bloodInventory.filter(item => calculateInventoryLevel(item) === 'critical').length}
                  </p>
                </div>
              ...
              {bloodInventory.map((item) => (
                <Card key={item.type} className="border-0 shadow-lg">
                  <CardHeader className="pb-3">
                    <div className="flex items-center justify-between">
                      <CardTitle className="text-2xl font-bold">{item.type}</CardTitle>
                      <Badge className={getStatusColor(calculateInventoryLevel(item))}>
                    ...
                    <div className="space-y-3">
                      <div>
                        <div className="flex justify-between items-center mb-1">
                          ...
                          <span className="text-lg font-bold">{item.units} units</span>
                        </div>
                        <Progress 
                      ...
                      <div className="grid grid-cols-2 gap-2 text-xs">
                        ...
                        <div>
                          ...
                          <span className="ml-1 font-medium">{item.critical}</span>
                        </div>
                      </div>
                      ...
                      {item.expiring > 0 && (
                        <Alert>
                          ...
                          <AlertDescription className="text-xs">
                            {item.expiring} unit{item.expiring > 1 ? 's' : ''} expiring within 3 days
                          </AlertDescription>
                        </Alert>
                      )}
                    ...
                    <div key={request.id} className="p-4 border rounded-lg">
                      <div className="flex items-center justify-between mb-3">
                        ...
                        <div className="flex items-center space-x-2">
                          ...
                          <Badge className={`${
                            request.status === 'fulfilled' ? 'bg-success/10 text-success' :
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-3">
                        ...
                        <div>
                          ...
                          <p className="font-semibold">
                            {format(new Date(request.requiredBy), 'MMM dd, HH:mm')}
                          </p>
                        </div>
                      </div>
                      ...
                      <div className="flex justify-end space-x-2">
                        {request.status === 'pending' && (
                          <>
                            ...
                            <Button size="sm" className="bg-hope-red hover:bg-hope-red/90">
                              Approve
                            </Button>
                          </>
                        )}
              ...
              <CardContent>
                <form onSubmit={handleRequestSubmit} className="space-y-6">
                  ...
                  <div className="flex justify-end space-x-3">
                    ...
                    <Button type="submit" className="bg-hope-red hover:bg-hope-red/90">
              ...
              <Card className="border-0 shadow-lg">
                ...
                <CardContent className="h-64 flex items-center justify-center">
                  <div className="text-center">
                    ...
                    <p className="text-muted-foreground">Chart: Blood usage trends over time</p>
                  </div>
                </CardContent>
              </Card>
              ...
              <Card className="border-0 shadow-lg">
                ...
                <CardContent className="h-64 flex items-center justify-center">
                  <div className="text-center">
                    ...
                    <p className="text-muted-foreground">Chart: Blood usage by department</p>
                  </div>
                </CardContent>
              </Card>
            </div>
